                ...
                {guarantees.map((item, idx) => (
                  <div key={idx} className="guarantee-item" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.6rem', flex: '1 1 160px', position: 'relative', zIndex: 1 }}>
                    ...
                    <div style={{ textAlign: 'center' }}>
                      <span className="guarantee-text" style={{ fontWeight: 800, fontSize: '1.1rem', display: 'block', marginBottom: '0.25rem', letterSpacing: '0.02em', color: 'white' }}>{item.text}</span>
                      <span style={{ fontSize: '0.7rem', fontWeight: 600, textTransform: 'uppercase', color: 'var(--color-plumbing-steel)', letterSpacing: '0.1em', display: 'block' }}>{item.subtext}</span>
                    </div>
                  </div>
                ))}
              </div>
